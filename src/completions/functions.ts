    ...
    public async provideCompletionItems(
        ...
        function getAdditionalEdits(
            ...
            for (const func of names) {
                ...
                varDecl += `local ${
                    func.varName ?? doCasing(func.importTitle)
                } = ${fusionData.name}.${func.name}${
        ...
        if (bracketMatch) {
            ...
            const items = Object.keys(
                fusionDefinitions[fusionVersion].functions
            )
                .filter((key) => {
                    return (
                        fusionDefinitions[fusionVersion].functions[key]
                            .isTableField &&
